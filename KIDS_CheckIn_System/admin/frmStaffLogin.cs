...
namespace KIDS_CheckIn_System.admin
{
    public partial class frmStaffLogin : Form
    {
        ...
        {
            if(lblNFC.Text =="NFC Code")
            {
                js.showExclamation("Please Scan Card");
                return;
            }

            DialogResult = System.Windows.Forms.DialogResult.OK;
            this.Tag = fldDesignation;
        ...
        private string GetOverRider(string nfc)
        {
            string overrider = "";

            string q = "SELECT fldAssignedTo FROM tblOverride WHERE fldOverrideCode='" + nfc + "'";
            ...
            if (js.RiD.HasRows)
            {
                overrider = "" + js.RiD["fldAssignedTo"].ToString();
            }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (CheckCard())
            {
                ...
                string overrider = GetOverRider(tmpStr);

                if (overrider != "")
                {
                    gbStaffName.Text = overrider;
                    lblNFC.Text = tmpStr;
                    fldDesignation = js.Lookup("fldDesignation","tblOverride","fldOverrideCode='" + tmpStr + "'");
                }
        ...
        private void frmStaffLogin_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if(mode=="")
                {
                    if (js.showQuestion("Do you want to Enable NFC Mode?") == System.Windows.Forms.DialogResult.Yes)
                    {
                    ...
                    js.showInformation("System will restart");
                    Application.Exit();
                }
